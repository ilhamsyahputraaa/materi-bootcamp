...
function App() {
  ...
  */
    ...
    const handleDelete = (id, name, age) => {
      console.log(id)
      Axios({
        method: 'post',
      ...
      <table>
        ...
        {data.map(item => {
          return <tr>
            ...
            <td>{item.name}</td>
            <td>{item.age}</td>
            <td><button onClick={() => handleEdit(item.id, item.name, item.age)}>Edit</button>
            <button onClick={() => handleDelete(item.id, item.name, item.age)}>Delete</button></td>
          </tr>
        })}
